...
class Vazao(variavel):
    ...
    unidadesMedida = [" ", "m^3/h", "m^3/min", "m^3/s",
                      "GPM", "BPH", "BPD", "pé^3/h", "pé^3/min"]
    ## Array de coeficientes para conversão de unidades
    coeficientes = [1, 0.016667, 0.00027778, 4.40287, 6.28982, 150.956, 35.314, 0.588579,
    ...
    def converter(self,vazao=None):  # Método para a conversão de Vazão
            ...
            self.imprimeTabela(unidadesMedida)
            ...
            conv = vazao * coeficientes[escolha-1]
            unSaida = escolha % 8
            print("Resultado da conversão: {0:.2f} {1} ".format(
                conv, unidadesMedida[8 if unSaida == 0 else unSaida]))

## Classe de vazão mássica que herda caracteristicas da classe vazão
class massica(Vazao):
    def calcular():  # Método para cálculo da vazão mássica
        ...
        if a == 1:
            ...
            if(int(input("\nPossui o valor da vazão volumetrica?: "))):
                ...
                self.vazvol = self.calcular()
            self.densidade = float(input("\nDigite o valor da densidade:"))
            vazao = vazvol*densidade

            print("O valor da Vazão Mássica é: {0} kg/s ", vazao)


## Classe de vazão volumétrica que herda caracteristicas da classe vazão
class volumetrica(Vazao):
    def calcular(self):  # Método para cálculo da vazão volumétrica
        tempo = 0.0
